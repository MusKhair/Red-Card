...
/** POST { playerName } — host-entry fallback for the Golden Boot winner (idempotent, first writer wins). */
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabase.rpc("enter_golden_boot_winner", { p_player_name: playerName.trim() });
  if (error) {
