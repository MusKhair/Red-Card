...
-- Lock cutoff is hardcoded to 2026-07-02 23:59:59 UTC (reopened for 1 day in stage15).
...
-- threshold is hit, or the host/proposer removes it (see RLS).
...
-- ============ TOURNAMENT AWARD RESOLUTION ============
-- FUZZY NAME MATCH (simplified — tournament-end resolution only). Lowercases, strips
-- non-alphanumeric chars (collapsing runs to single spaces), trims. Diacritics are NOT
-- stripped (no unaccent dependency) — acceptable for a one-time, host-verified entry.
-- Two names match if equal after normalization, or if their last tokens (surnames)
-- match (e.g. "Mbappe" ~ "Kylian Mbappe").
...
-- resolve_tournament_awards: security definer, idempotent. Called from the daily sync
-- cron (and a manual API route). No-op per award if that award is already resolved.
--
...
-- Immediately recomputes golden_boot_points for everyone if this call is the one that
-- resolved it.
